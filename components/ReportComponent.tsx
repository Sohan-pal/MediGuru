...
const ReportComponent = ({ onReportConfirmation }: Props) => {
  ...
  const [requestQueue, setRequestQueue] = useState<boolean>(false);
  const RETRY_DELAY = 120000; // Increase to 2 minutes
  ...
  function compressImage(file: File, callback: (compressedFile: File) => void) {
    ...
    reader.onload = (e) => {
      ...
      img.onload = () => {
        ...
        const quality = 0.5; // Adjust based on your needs
  ...
  async function extractDetails(): Promise<void> {
    ...
    setIsLoading(true);
    toast({
      description: "Processing image... Trying multiple services...",
      duration: 10000,
    });
    ...
    try {
      ...
      if (!response.ok) {
        throw new Error(
          data.details || data.error || "Failed to process image"
        );
      }
